...
$app->get('/exercise/part1',function() use($app) {
    return $app->render('exercise_part1.twig');
});

$app->get('/exercise/part2',function() use($app) {
    $con = $app->db;
    $sql = 'select birthday,sex from  user use index(idx3) where id = ?';
    $sth = $con->prepare($sql);
    $sth->execute(array(mt_rand(1,1000007)));
    $result = $sth->fetch(PDO::FETCH_BOTH);
    return $app->render('exercise_part2.twig',['birthday' => $result['birthday'],'sex' => $result['sex']===1?'男':'女']);
});
...
$app->get('/exercise/part4',function() use($app) {
    ...
    return $app->render('exercise_part4.twig',['messages' => $results]);
});
